...
function LiveProjectCard({
  ...
  project: (typeof profileData.liveProjects)[0];
  index: number;
  isExpanded: boolean;
  onToggle: () => void;
}) {
  const cardRef = useRef<HTMLDivElement>(null);
  const borderGradientClass = "project-border-blue"; // Keep it consistent or specific for live app
  ...
  return (
    ...
    >
      <TiltCard className="h-full">
        ...
        <div
          ref={cardRef}
          data-bird-target={`live-project-${project.id}`}
        ...
        >
          ...
          <div className="absolute inset-0 opacity-[0.015] mix-blend-overlay pointer-events-none">
            <svg className="w-full h-full">
              <filter id={`noise-live-${project.id}`}>
                <feTurbulence
                  type="fractalNoise"
                  baseFrequency="0.8"
                  numOctaves="4"
              ...
              <rect
                width="100%"
                height="100%"
                filter={`url(#noise-live-${project.id})`}
              />
            </svg>
          </div>
...
export default function LiveProjectsSection() {
  ...
  // If no live projects, don't render section
  if (!profileData.liveProjects || profileData.liveProjects.length === 0)
    return null;

  return (
    <section id="live-projects" className="relative py-20 px-6 overflow-hidden">
      {/* Background Elements - distinct from other sections */}
      <div className="absolute inset-0 overflow-hidden z-0">
        <div className="absolute inset-0 bg-gradient-to-b from-blue-900/10 to-transparent opacity-30" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[500px] bg-blue-500/5 blur-[100px] rounded-full pointer-events-none" />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6">
            <span className="block">EduNexus</span>
          ...
          <p className="text-gray-400 text-lg md:text-xl max-w-2xl leading-relaxed">
            ...
            serving real users. Bridging the gap between academic preparation
            and professional engineering skills.
          </p>
        </motion.div>

        {/* Live Projects Grid (usually just one, but scalable) */}
        <div className="grid grid-cols-1 gap-8">
          {profileData.liveProjects.map((project, index) => (
            <LiveProjectCard
              key={project.id}
              project={project}
              index={index}
